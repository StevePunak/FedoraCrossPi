...
def uninstall(app_id: str) -> None:
    ...
    _remove_mount_units(record.manifest)
    _systemctl("daemon-reload", check=False)
...
def _write_mount_units(record: InstalledApp) -> None:
    """Render .mount and (when MountSpec.automount=true) .automount unit
    files for every declared mount. Credential files referenced inside
    `options=` are the operator's responsibility — see app_mount module
    docstring."""
...
def _remove_mount_units(manifest: AppManifest) -> None:
